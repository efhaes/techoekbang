# ekbang/views/koperasi/kecamatan.py
from django.shortcuts import render, redirect, get_object_or_404
from django.http import JsonResponse
from django.contrib.auth.decorators import login_required
from django.contrib import messages
from django.core.exceptions import ValidationError
from django.db.models import Count, Q
from ekbang.models import Koperasi, Desa                     # tambah Desa
from ekbang.decorators import kecamatan_required
from ekbang.views.auth import get_tahun_context


@login_required
@kecamatan_required
def koperasi_list_kecamatan(request):
    status_filter     = request.GET.get('status', '')
    desa_filter       = request.GET.get('desa', '')          # tambah
    tahun, tahun_list = get_tahun_context(request)

    # Get all desa and add "Semua Desa" option
    desa_objects = list(Desa.objects.all())
    semua_desa = type('Desa', (), {
        'pk': 0,
        'nama': 'Semua Desa'
    })()
    desa_list = desa_objects + [semua_desa]

    return render(request, 'kecamatan/koperasi/list.html', {
        'status_filter': status_filter,
        'desa_filter':   desa_filter,                        # tambah
        'desa_list':     desa_list,                            # tambah
        'tahun':         tahun,
        'tahun_list':    tahun_list,
    })


@login_required
@kecamatan_required
def koperasi_filter_api(request):
    tahun = request.GET.get('tahun')
    desa_id = request.GET.get('desa_id')
    status_filter = request.GET.get('status', '')

    if not tahun:
        return JsonResponse({'error': 'Parameter tahun wajib diisi'}, status=400)

    # Handle "Semua Desa" case (desa_id = 0)
    if desa_id == '0':
        qs = Koperasi.objects.select_related('desa', 'diajukan_oleh').filter(
            tahun_anggaran=tahun
        ).exclude(status='draft').order_by('-tanggal_diajukan')
        
        summary_qs = Koperasi.objects.filter(tahun_anggaran=tahun).exclude(status='draft')
    else:
        qs = Koperasi.objects.select_related('desa', 'diajukan_oleh').filter(
            tahun_anggaran=tahun,
            desa_id=desa_id
        ).exclude(status='draft').order_by('-tanggal_diajukan')
        
        summary_qs = Koperasi.objects.filter(
            tahun_anggaran=tahun, desa_id=desa_id
        ).exclude(status='draft')

    if status_filter in ('diajukan', 'disetujui', 'ditolak'):
        qs = qs.filter(status=status_filter)

    summary = summary_qs.aggregate(
        total_diajukan=Count('id', filter=Q(status='diajukan')),
        total_disetujui=Count('id', filter=Q(status='disetujui')),
        total_ditolak=Count('id', filter=Q(status='ditolak')),
    )

    data = []
    for obj in qs:
        data.append({
            'pk': obj.pk,
            'desa_nama': obj.desa.nama,
            'nama_koperasi': obj.nama_koperasi,
            'tanggal_diajukan': obj.tanggal_diajukan.strftime('%d %b %Y') if obj.tanggal_diajukan else '-',
            'status': obj.status,
            'status_display': obj.get_status_display()
        })

    # Debug info
    print(f"DEBUG KOPERASI: tahun={tahun}, desa_id={desa_id}, status_filter={status_filter}")
    print(f"DEBUG KOPERASI: qs.count()={qs.count()}")
    print(f"DEBUG KOPERASI: summary={summary}")
    print(f"DEBUG KOPERASI: data length={len(data)}")

    return JsonResponse({
        'data': data,
        'summary': summary
    })


@login_required
@kecamatan_required
def koperasi_detail_kecamatan(request, pk):
    obj = get_object_or_404(
        Koperasi.objects.select_related(
            'desa', 'diajukan_oleh', 'diverifikasi_oleh'
        ),
        pk=pk,
    )
    if obj.status == 'draft':                                # tambah guard
        messages.error(request, "Data belum diajukan oleh desa.")
        return redirect('koperasi_list_kecamatan')
    return render(request, 'kecamatan/koperasi/detail.html', {'obj': obj})


@login_required
@kecamatan_required
def koperasi_verifikasi(request, pk):
    if request.method != 'POST':
        return redirect('koperasi_detail_kecamatan', pk=pk)

    obj     = get_object_or_404(Koperasi, pk=pk)
    status  = request.POST.get('status', '').strip()
    catatan = request.POST.get('catatan_verifikasi', '').strip()

    if status not in ('disetujui', 'ditolak'):
        messages.error(request, "Status verifikasi tidak valid.")
        return redirect('koperasi_detail_kecamatan', pk=pk)

    if status == 'ditolak' and not catatan:
        messages.error(request, "Alasan penolakan wajib diisi.")
        return redirect('koperasi_detail_kecamatan', pk=pk)

    try:
        obj.verifikasi(request.user, status, catatan)
        if status == 'disetujui':
            messages.success(request, f"Koperasi {obj.nama_koperasi} Desa {obj.desa.nama} telah disetujui.")
        else:
            messages.warning(request, f"Koperasi {obj.nama_koperasi} Desa {obj.desa.nama} telah ditolak.")
    except ValidationError as e:
        messages.error(request, str(e.message))

    return redirect('koperasi_list_kecamatan')