...
def bumdes_filter_api(request):
    ...
    if desa_id == '0':
        ...
        summary_qs = Bumdes.objects.filter(tahun_anggaran=tahun).exclude(status='draft')
    else:
        qs = Bumdes.objects.select_related('desa').filter(
            tahun_anggaran=tahun,
            desa_id=desa_id
        ...
        summary_qs = Bumdes.objects.filter(tahun_anggaran=tahun, desa_id=desa_id).exclude(status='draft')
