# ekbang/views/infrastruktur/kecamatan.py
...
from ekbang.models import Infrastruktur, Desa
...
@login_required
@kecamatan_required
def infrastruktur_list_kecamatan(request):
...
@login_required
@kecamatan_required
def infrastruktur_filter_api(request):
    ...
    # Debug info
    print(f"DEBUG INFRASTRUKTUR: tahun={tahun}, desa_id={desa_id}, status_filter={status_filter}")
    print(f"DEBUG INFRASTRUKTUR: qs.count()={qs.count()}")
    print(f"DEBUG INFRASTRUKTUR: summary={summary}")
    print(f"DEBUG INFRASTRUKTUR: data length={len(data)}")
...
@login_required
@kecamatan_required
def infrastruktur_detail_kecamatan(request, pk):
    obj = get_object_or_404(
        Infrastruktur.objects.select_related(
            'desa', 'diajukan_oleh', 'diverifikasi_oleh'
        ).prefetch_related('details', 'fotos'),
        pk=pk,
    )
    if obj.status == 'draft':                  # tambah guard
        messages.error(request, "Data belum diajukan oleh desa.")
        return redirect('infrastruktur_list_kecamatan')
    return render(request, 'kecamatan/infrastruktur/detail.html', {'obj': obj})


@login_required
@kecamatan_required
def infrastruktur_verifikasi(request, pk):
    if request.method != 'POST':
        return redirect('infrastruktur_detail_kecamatan', pk=pk)

    obj     = get_object_or_404(Infrastruktur, pk=pk)
    ...
    try:
        ...
        if status == 'disetujui':
            messages.success(request, f"Infrastruktur Desa {obj.desa.nama} telah disetujui.")
        else:
            messages.warning(request, f"Infrastruktur Desa {obj.desa.nama} telah ditolak.")
