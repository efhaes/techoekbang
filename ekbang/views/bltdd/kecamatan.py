...
def bltdd_filter_api(request):
    ...
    if desa_id == '0':
        ...
        summary_qs = BLTDD.objects.filter(tahun_anggaran=tahun).exclude(status='draft')
    else:
        qs = BLTDD.objects.select_related('desa', 'diajukan_oleh').filter(
            tahun_anggaran=tahun,
            desa_id=desa_id
        ...
        summary_qs = BLTDD.objects.filter(tahun_anggaran=tahun, desa_id=desa_id).exclude(status='draft')
